...
#
# THE 100% BUG:
# Old code trained on rule-generated labels, tested on the
# same rule-generated labels → model just memorises the rules → 100%.
#
# Fix:
# 1. Train ONLY on DSAT tickets (strongest label signal)
# 2. Use 5-fold STRATIFIED cross-validation → each fold is
#    tested on data it was NEVER trained on → honest score
# 3. Final model trained on full labeled set for live inference
# ─────────────────────────────────────────────
...
st.caption(f"Last week ({str(prev_week)[:10]}) → This week ({str(curr_week)[:10]}) · All agents combined · Sorted by biggest DSAT increase first")
...
st.markdown('<div class="sub-header">📦 Products — DSAT This Week vs Last Week</div>', unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
        ...
        if not ag_all[ag_all['Product']==primary_prod].empty:
            pw = ag_all[ag_all['Product']==primary_prod].groupby('Week').agg(DSAT_count=('DSAT','sum')).reset_index().sort_values('Week')
            st.caption(f"📈 Weekly DSAT — {primary_prod} · {agent}")
            st.line_chart(pw.set_index('Week')[['DSAT_count']].rename(columns={'DSAT_count':f'{primary_prod} DSAT'}))

        # ── All products ranked
        st.markdown('<div class="sub-header">📦 All Products — DSAT Ranked + WoW for This Agent</div>', unsafe_allow_html=True)
        for prod, total_d in prod_dsat_totals.items():
            cd = int(ag_all[(ag_all['Week']==curr_week)&(ag_all['Product']==prod)&(ag_all['DSAT']==1)].shape[0])
            pd_ = int(ag_all[(ag_all['Week']==prev_week)&(ag_all['Product']==prod)&(ag_all['DSAT']==1)].shape[0])
            dd  = cd - pd_
            ct  = int(ag_all[(ag_all['Week']==curr_week)&(ag_all['Product']==prod)].shape[0])
            pt  = int(ag_all[(ag_all['Week']==prev_week)&(ag_all['Product']==prod)].shape[0])

            fc2 = ag_all[(ag_all['Week']==curr_week)&(ag_all['Product']==prod)&(ag_all['DSAT']==1)]['Feature'].value_counts()
            fp2 = ag_all[(ag_all['Week']==prev_week)&(ag_all['Product']==prod)&(ag_all['DSAT']==1)]['Feature'].value_counts()
            frows = []
            for f in sorted(set(fc2.index)|set(fp2.index)):
                fcc=int(fc2.get(f,0)); fpp=int(fp2.get(f,0)); fd=fcc-fpp
                c_="#f87272" if fd>0 else ("#34d399" if fd<0 else "#8b92ab")
                a_="⬆" if fd>0 else ("⬇" if fd<0 else "➡")
                frows.append(f"<span style='color:#8b92ab'>{f}:</span> <span style='color:{c_}'>{a_} {fcc} ({fd:+d})</span>")
            fhtml = " &nbsp;·&nbsp; ".join(frows) or "<span style='color:#5a6484'>No DSAT this week</span>"

            ppp2, tot2 = ppp_counts(ag_all[(ag_all['Product']==prod)&(ag_all['DSAT']==1)]['Combined_Text'])
            ppills = "".join([
                f"<span class='ppp-pill {cls}'>{cat}: {ppp2.get(cat,0)} ({round(ppp2.get(cat,0)/tot2*100) if tot2>0 else 0}%)</span>"
                for cat,cls in [("People","pill-people"),("Process","pill-process"),("Product","pill-product")]
            ])
            border = "#4f7bf7" if prod==primary_prod else "#232d45"
            star   = "⭐ Primary &nbsp;" if prod==primary_prod else ""

            st.markdown(f"""
<div class="product-story" style="border-color:{border}">
  <div class="prod-name">📦 {prod}
    <span style="color:#facc15;font-size:0.78rem">{star}</span>
  ...
  <span style="color:#c9d1e8;font-size:0.85rem"> {ct} ({ct-pt:+d})</span><br><br>
  <span style="color:#5a6484;font-size:0.76rem;text-transform:uppercase">Feature DSAT — this week vs last</span><br>
  <span style="font-size:0.84rem">{fhtml}</span><br><br>
  <div class="ppp-row">{ppills}</div>
</div>
""", unsafe_allow_html=True)
        ...
        for txt in ag_dsat['Combined_Text']:
            for kw in KW_MAP.get(dominant_cat,[]):
                if kw in str(txt).lower(): matched.append(kw); break
        if matched:
            kw_df = pd.DataFrame(Counter(matched).most_common(8), columns=["Keyword","Occurrences"])
            st.caption(f"Top keywords driving **{dominant_cat}** complaints")
